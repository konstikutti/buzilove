  ...
  return (
    <div className="space-y-4">
      <div
        ...
        className={`border-2 border-dashed border-slate-300 rounded-xl p-6 flex flex-col items-center justify-center transition-colors ${loading ? 'cursor-wait opacity-70' : 'cursor-pointer hover:border-indigo-500 hover:bg-indigo-50'}`}
      >
        <input
          type="file"
          multiple
        ...
        {loading ? (
          <div className="flex flex-col items-center">
             ...
             <span className="text-[10px] text-slate-400 mt-1">Bitte warten...</span>
          </div>
        ) : (
          <>
...
const MemoryDetail = ({ memory, onBack, onEdit, isAuthor }) => {
  ...
  const contentBlocks = useMemo(() => {
      ...
      if (!memory.blocks) return [{ id: 1, type: "text", content: memory.content || "" }];
  ...
  const currentHeroImage = hydratedImages.length > 0 && hydratedImages[0] 
    ? hydratedImages[0] 
    : (memory.previewImage || "");
    ...
    <div className={`min-h-screen ${getBg()}`}>
      ...
      {loadedCount < totalImages && (
        <div className="fixed bottom-4 right-4 bg-white/90 backdrop-blur border shadow-lg p-3 rounded-full flex items-center gap-3 text-xs font-bold text-indigo-600 z-50 animate-in slide-in-from-bottom-4">
            ...
            <span>Lade Fotos ({loadedCount}/{totalImages})...</span>
        </div>
      )}
